...
def _strptime(data_string: str, format: str) -> tuple:
    ...
    with _cache_lock:
        ...
        if not format_regexes:
            ...
            # TODO: not sure these are the right exceptions to catch after refactoring
            ...
            _regex_cache[format] = format_regexes

    for format_regex in format_regexes:
        # try to match the regex to the data_string
        try:
            results = _strptime_regex(data_string, format_regex, locale_time)
        except ValueError:
            continue
        else:
            return results
...
def _build_regex_patterns(s: str, time_re: TimeRE) -> list[str]:
    """Build regex patterns from strptime format string"""
    try:
        model = STRPTIME_META_MODEL.model_from_str(s)
    ...
    for pattern in model.format_patterns:
        ...
        for fs in pattern.format_strings:
            ...
            if match := RE_SKIP_CHARS.match(fs.format_code):
                regex += ".{" + match.group(1) + "}"
            elif match := RE_SPECIAL_CHARS.match(fs.format_code):
